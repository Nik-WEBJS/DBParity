...
- [ ] Сегментные DB-side хэши: разбиение по PK-диапазонам, сравнение агрегатов,
  спуск только в расходящиеся сегменты. Ограничение: хэш на стороне БД обходит
  клиентскую нормализацию → применять только к «безопасным» типам, остальные
  колонки — клиентски (гибридный режим)
