...
> **Статус: v0.3 alpha.** Ядро и PostgreSQL-адаптер проверены на живом
> PostgreSQL 18. Oracle-адаптер написан, но не обкатан в бою —
...
Checkpoint/resume после обрыва → сетевые retry → обкатка Oracle/MSSQL с
сообществом → parallel-run для dual-write переключений → v1.0 с замороженными
форматами. Подробно: [ROADMAP.md](ROADMAP.md).
